...
'''
    ...
    Copyright (C) 2017 Tao Lei under the Apache License 2.0
    Modifications 2017 Michael Bühler
'''
...
class EmbeddingLayer(object):
    ...
    def __init__(self, n_d, vocab, oov="<unk>", embs=None, fix_init_embs=True):
        ...
        self.embeddings = create_shared(emb_vals.astype(theano.config.floatX))
        if self.init_end > -1:
...
class LSTM(Layer):
    '''
        LSTM implementation.
    '''
    def __init__(self, n_in, n_out, activation=tanh, clip_gradients=False, dropout = None):
        self.n_in = n_in
        self.n_out = n_out
        self.activation = activation
        self.clip_gradients = clip_gradients
    ...
    def forward(self, x, hc):
        ...
        #    h_tm1 = apply_dropout(h_tm1, self.dropout)
    ...
    def forward_all(self, x, steps, h0=None, return_c=False):
        '''
            ...
            steps     : The number of LSTM units which should be processed before the scan operation stops.
                          (if 'n_steps == -1', then the sequence x will be processed until the end.)
