...
class Generator:
    def __init__(self, args, dropout, dim_sent_embedding, dim_flic_embedding):
        ...
        # Construct Transformation Layer depending on the context flag (FF layers).
        ...
        for i in range(num_layers):
            if i == 0 and i == num_layers - 1:
                if self.args.context == 'train_context':
                    ...
                    self.ff_layers.append(Layer(self.dim_sent_emb, self.dim_flic, activation=activation))
            elif i == 0 and i != num_layers - 1:
                if self.args.context == 'train_context':
                    self.ff_layers.append(Layer(self.dim_flic, num_units, activation=activation))
                else:
                    self.ff_layers.append(Layer(self.dim_sent_emb, num_units, activation=activation))
            elif i != 0 and i == num_layers -1 :
                if self.args.context == 'train_context':
                    self.ff_layers.append(Layer(num_units, self.dim_sent_emb, activation=activation))
                else:
                    self.ff_layers.append(Layer(num_units, self.dim_flic, activation=activation))
            else:
    ...
    def transform_context(self, context, normalize_embeddings=True):
        """ Apply the FF layers to the context vector

        :type context: T.vector
        :param context: The context vector
        :type normalize_embeddings: Boolean
        :param normalize_embeddings: If yes, the length of the context is rescaled to 1.

        :type return value: T.vector
        :return return value: The transformed context vector
        """
        if self.args.context == 'train_context':
    ...
    def transform_sent_embs(self, sent_embs, normalize_embeddings=True):
        """ Apply the FF layers to the sentence embedding vectors

            :type sent_embs: T.matrix
            :param sent_embs: The matrix containing the sentence embeddings in the rows
            :type normalize_embeddings: Boolean
            :param normalize_embeddings: If yes, the length of each embedding is rescaled to 1.

            :type return value: T.matrix
            :return: return value: The transformed sentence embedding vectors

        """

        if self.args.context != 'train_context':
            for layer in self.ff_layers:
                sent_embs = layer.forward(sent_embs)
                if self.dropout:
                    sent_embs = apply_dropout(sent_embs, self.dropout)

            if normalize_embeddings:
                sent_embs_normalized = sent_embs / T.sqrt((sent_embs ** 2).sum(axis=1)).reshape((sent_embs.shape[0], 1))
            else:
                sent_embs_normalized = sent_embs
        else:
            sent_embs_normalized = sent_embs
        return sent_embs_normalized

    def get_L(self, sent_embs, context):
        """ Get the Ensemble matrix L
        ...
        """
        if self.args.context == 'no_context':
            L = self.dpp.build_L_const_context(sent_embs, activation=tanh, biased_diagonal=False)
        else:
            L = self.dpp.build_L(sent_embs, context, activation=tanh, biased_diagonal=False)
        L = T.abs_(self.scale_sent_embs) * L
        return L

    def sample(self, L, num_samples):
        """ Sampling from the DPP model given by L.

        :type L: np.array (ndim == 2)
        :param L: The Ensemble matrix
        :type num_samples: int
        :param num_samples: The number of samples which should be returned

        :type return value: list of 'num_samples' numpy arrays with shape (np.shape(L)[0],)
        :return return value: a list of sampled sets. Each set is described by an indicator array, where the i-th entry
        is one iff the i-th element is in the sampled set.
    ...
    def expected_set_size(self, L):
        """ Expected set size of the DPP model given by L

        :type L: np.array (ndim == 2)
        :param L: L-Ensemble matrix

        :type return value: float
        :return return value: Expected set size
