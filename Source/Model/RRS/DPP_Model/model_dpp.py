from generator_dpp import Generator
...
class Model:
    def __init__(self, args, sent_emb_dim, flic_dim, load_model=None, epochs_done=0):
        ...
        transformed_sent_embs_t = self.generator.transform_sent_embs(sent_embs_t, normalize_embeddings=True)

        if not self.args.sample_all_sentences:
            # Construct L to sample from the DPP.
            L_t = self.generator.get_L(transformed_sent_embs_t, transformed_context_t)
            ...
            padded_sents_t, sents_count_t = self.generator.get_padded_embeddings_from_samples_t(samples_t, transformed_sent_embs_t, max_num_sents_t)
        ...
        # TODO: Implement or remove
            ...
            probs_mean_t, costs_encoder_mean_t = self.encoder.cost_all_sentences(transformed_sent_embs_t, transformed_context_t, y_t)
    ...
    def testset_evaluation(self, D_test, D_C_test, prod_to_sent, flic, num_samples = 20, return_all_probs=False):
        ...
        for context, x, y in training_sample(D_test, D_C_test):
            ...
            if not self.args.sample_all_sentences:
                ...
                if return_all_probs:
                    sampled_sentences.append((context, x, y, mean_prob, get_strings_from_samples(prod_to_sent[x][1], samples)))
                else:
